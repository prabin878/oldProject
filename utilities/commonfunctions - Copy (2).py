...
class CommonFunctions:
   ...
   # Deletes the test result file. FilePath with Filename needs to be provided
            ...
            if (os.path.getsize(file) > 0):
                with open(file) as f:
                    try:
                        ...
                        logging.error ("File "+fname +" has invalid json structure")
                        val = False
            else:
                val= False 
    ...
    # Checks for the API Status and if it does not match with expected value then returns dict obj with error 
    def processAPIStatus(rs,expectedStatus):
        results={}
        if (rs.status_code!= expectedStatus) :
            assertStatus="expected:" + str(expectedStatus) + ", actual:"+ str(rs.status_code)
            results["error"] = assertStatus
            apiPass=False
        else:
            assertStatus = 'Response matching with expected value'
            results["apiPass"] = True
        return results

    #If there is error with RequestPayload File (function getJson) then returns dict object with error msg
    def processPayloadFileError(json_struct, tcno, ban):
        results={}
        if (json_struct==None) :
            assertStatus=tcno +":JSON file for "+str(ban)+"- either file does not exist or is blank or has invalid json or ban/conversion id is not matching"
            results["reason"] = assertStatus
        return results

    # Happy path1- if payload files are good and no API status error then validate the Api response fields
    def processPayloadFields(resBody,expectedBody, validateFields):
        assertField={}
        results={}
    ...
    # Happy path2- if expected status is matching and payloadfile has response=blank then testcase passes
    # eg expStatus = 404 and actual Status=404 (anything other than success)
    def processPayloadExpRespBodyEmpty(resBody,expectedBody):
        expRespBodyBlank=False
        if (expectedBody=={}):
            assertStatus= "Expected status matches Actual status and Expected response is blank.. so do nothing"
            expRespBodyBlank=True
        return expRespBodyBlank

    def processPayloadConversionId(payloadConv, expConv):
        results={}
        isValidConv= True
        if (payloadConv!=expConv)or (payloadConv == None) :
            logging.error("expected conversionId: "+ expConv +" does not match with conversionId from payload: "+ payloadConv)
            isValidConv= False
        else:
            # expected Conversion id matches with conversion id in Payload. Do nothing
            isValidConv= True
            results['isValidConv']= True
        return isValidConv
